    ...
    def evaluate_models(self, X,y,models):
        try:
            ...
            for i in range(len(list(models))):
                ...
                train_model_score= accuracy_score(X_train , y_train_pred)
        ...
        def get_best_model(self,x_train:np.array, y_train:np.array, x_test:np.array, y_test:np.array):
            try:
                ...
                best_model_name = lsit(model_report.keys())[list(model_report.values()).index(best_model_score)]
        ...
        def finetune_best_model(self ,best_model_object:object,best_model_name ,X_train , y_train)->object:
            try:
               ...
               grid_search =GridSearchCV(
                   best_model_object, params_grid=model_param_grid,cv=5,n_jobs=-1,verbose=1)
        ...
        def initiate_model_trainer(self, traun_array, test_array):
            try:
                logging.info(f"Splitting training and testing input and target freature")
